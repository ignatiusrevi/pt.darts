...
from models import ops_cka


class SearchCellCKA(nn.Module):
    """ Cell for search
    ...
    def __init__(self, n_nodes, C_pp, C_p, C, reduction_p, reduction):
        ...
        if reduction_p:
            self.preproc0 = ops_cka.FactorizedReduce(C_pp, C, affine=False)
        else:
            self.preproc0 = ops_cka.StdConv(C_pp, C, 1, 1, 0, affine=False)
        self.preproc1 = ops_cka.StdConv(C_p, C, 1, 1, 0, affine=False)
        ...
        for i in range(self.n_nodes):
            ...
            for j in range(2+i): # include 2 input nodes
                ...
                op = ops_cka.MixedOpCKA(C, stride)
