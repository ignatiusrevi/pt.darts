...
def main():
    ...
    supernet = SearchCNNController(C_in=input_channels, C=16, n_classes=n_classes, n_layers=8, criterion=net_crit, device_ids=device_ids)
    childnet = AugmentCNN(input_size=input_size, C_in=input_channels, C=16, n_classes=10, n_layers=20, auxiliary=True, genotype=gt.from_str("Genotype(normal=[[('skip_connect', 0), ('sep_conv_3x3', 1)], [('dil_conv_3x3', 1), ('skip_connect', 0)], [('sep_conv_3x3', 0), ('dil_conv_5x5', 1)], [('sep_conv_3x3', 0), ('sep_conv_3x3', 2)]], normal_concat=range(2, 6), reduce=[[('max_pool_3x3', 0), ('skip_connect', 1)], [('skip_connect', 2), ('max_pool_3x3', 0)], [('skip_connect', 2), ('max_pool_3x3', 0)], [('dil_conv_5x5', 4), ('skip_connect', 2)]], reduce_concat=range(2, 6))"))
    childnet = nn.DataParallel(childnet, device_ids=device_ids).to(device)

    utils.load(supernet, './results/cifar10/weights.pt')
    utils.load(childnet, './results/cifar10-l20-init16-64-childnet/weights.pt')
    supernet.to(device)
    childnet.to(device)
    ...
    for i in range(n_cells): # 8 Cells
        ...
        if i not in [n_cells//3, 2*n_cells//3]:
            ...
            for j in range(n_ops):
                if j != 2: # skip_connect
                    supernet.net.cells[i].dag[0][0]._ops[j] = ops.Zero(stride=1)
            for j in range(n_ops):
                if j != 3: # sep_conv_3x3
                    supernet.net.cells[i].dag[0][1]._ops[j] = ops.Zero(stride=1)

            # DAG 1
            for j in range(n_ops):
                if j != 2: # skip_connect
                    supernet.net.cells[i].dag[1][0]._ops[j] = ops.Zero(stride=1)
            for j in range(n_ops):
                if j != 5: # dil_conv_3x3
                    supernet.net.cells[i].dag[1][1]._ops[j] = ops.Zero(stride=1)
            for j in range(n_ops):
        ...
        # Reduction Cell
            ...
            for j in range(n_ops):
                if j != 1: # max_pool_3x3
                    supernet.net.cells[i].dag[2][0]._ops[j] = ops.Zero(stride=2)
            for j in range(n_ops):
                if j != 2: # skip_connect
                    supernet.net.cells[i].dag[2][2]._ops[j] = ops.Zero(stride=1)
            w = [1, 3]
            for k in w:
                if k == 1:
                    stride = 2
            ...
            for j in range(n_ops):
                if j != 2: # skip_connect
                    supernet.net.cells[i].dag[3][2]._ops[j] = ops.Zero(stride=1)
            for j in range(n_ops):
                if j != 6: # dil_conv_5x5
